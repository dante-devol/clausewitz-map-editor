...
// Each editor mode remembers its own last-used map mode, so switching
// editor modes restores whichever map mode was last active there instead of
// leaving whatever map mode happened to be active in the previous editor
// mode. These are the defaults the first time a given editor mode is
// entered in a session.
const DEFAULT_DISPLAY_MODE_BY_EDITOR_MODE: Record<EditorMode, DisplayMode> = {
  provinces: 'provinces',
  states: 'state',
...
export function useMapViewportState() {
  ...
  const rememberedDisplayModeByEditorModeRef = useRef<Record<EditorMode, DisplayMode>>({
    ...DEFAULT_DISPLAY_MODE_BY_EDITOR_MODE
  })
  ...
  // and restore whichever map mode was last used in the editor mode we're
  // entering (remembering the one we're leaving first).
  useEffect(() => {
    ...
    rememberedDisplayModeByEditorModeRef.current[previousEditorMode] = displayMode
    const remembered = rememberedDisplayModeByEditorModeRef.current[editorMode]
      ?? DEFAULT_DISPLAY_MODE_BY_EDITOR_MODE[editorMode]
    if (remembered !== displayMode) dispatchDisplayMode(remembered)
  ...
  }, [editorMode, displayMode, dispatchDisplayMode, setPaintActiveTool])
