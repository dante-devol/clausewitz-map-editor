...
export function MapCanvas(): JSX.Element {
  ...
  // Strokes only update the in-memory pixel buffer and the pending-strokes
  // list here — persisting to provinces.bmp is explicit (see the save-request
  // effect below), triggered by the paint panel's save button.
  ...
  // Runs an explicit BMP save when PaintPanel's save button bumps
  // bmpSaveRequestId. MapCanvas holds the live pixel buffer, so it's the only
  // place that can produce a snapshot to write.
  useEffect(() => {
    if (bmpSaveRequestId === 0) return
    let cancelled = false

    async function save() {
      if (!projectId) {
        bmpSaveFailed(t('paintPanel.save.error'))
        return
      }
      ...
      try {
        const rgba = new Uint8Array(snapshot.data.buffer, snapshot.data.byteOffset, snapshot.data.byteLength)
        await window.api.map.saveBmp(projectId, rgba, snapshot.width, snapshot.height)
        if (cancelled) return
        bmpSaveSucceeded()
      } catch (error) {
        if (cancelled) return
        bmpSaveFailed(error instanceof Error ? error.message : t('paintPanel.save.error'))
      }
    }

    void save()
    return () => { cancelled = true }
  }, [bmpSaveRequestId, projectId, bmpSaveSucceeded, bmpSaveFailed, t])
