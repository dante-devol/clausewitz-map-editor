...
> **Status (2026-09-11): section 4 fully addressed — every numbered item, including both halves of 4.3 and both halves of 4.4.**
...
> - 4.3: the `createImageBitmap`+canvas decode duplicated across `BmpProvinceMapSource` (used by both `useMapCanvas` and `useOverlayAssets`) and the bitmap-analysis worker is replaced by the pure `decodeBmp24` decoder (`decodeBmp24.ts`), removing the GPU/canvas round-trip and its possible color-management shift on province colors. `BmpProvinceMapSource` caches the parsed bytes by source array reference so loading the same image twice only parses once, while still handing each caller an independent pixel-array copy (`MapRenderer` mutates its copy in place while painting, so a shared array would leak edits between callers). `imageBitmap` is dropped from `ProvinceMapSource` entirely — it was decoded in both call sites but never actually consumed by either. The wire format is also fixed: `MapDataSnapshot.provincesImage`/`ImageChangedData.data` are now `Uint8Array`, sent as-is over Electron IPC (which structured-clones typed arrays directly) instead of `.toString('base64')` on the main side and a rebuilt `data:image/bmp;base64,...` string on the renderer side — no base64 anywhere in the path anymore.
> - 4.4: `selectEffectiveProvinceCatalog` now caches the same way as 4.1, so `useProvinceEditTargets` and `useProvinceValidation` no longer each rebuild the whole catalog (including the id-gap scan) on the same render. Validator execution itself also moved off the main thread: `runValidationForCatalog`/`runValidationForProvince` were pulled into a pure module (`runProvinceValidationPass.ts`) that `provinceValidation.worker.ts` runs, and `useProvinceValidation` accumulates changed province keys across a 250ms debounce window before dispatching one worker request — a burst of rapid edits now triggers one validation pass instead of one per edit, and the pass itself never blocks the main thread. A `requestId` discards a worker reply superseded by a newer edit.
...
> - 4.5, painted-index staleness: `paintBrush`/`revertBrushStroke` now call `updateProvinceBboxesForRegion` (`provinceAnalysis.ts`) after mutating `idData`, keeping bboxes correct without a full-image rescan (scanning each affected id's old bbox unioned with the changed region is exact, since the old bbox already bounded every pixel that could still belong to it).
> - A dev-only test harness (`docs/testing-issue-4.md`) runs the real renderer and main-process loader against a small, real HOI4-derived fixture in a plain browser tab. It's how every fix above was actually verified — DPR-2 numeric checks, click-to-select accuracy, a live `WEBGL_lose_context` round-trip, painting a stroke and watching the selection outline shrink/grow correctly, and confirming the province-outline overlay stays pixel-aligned with the main map through the new decode path — rather than just reasoned about from source.
> - Tests: `npm test` (`provinceEditSelectors.test.ts`, `useMapCanvas.mergeBboxGroups.test.ts`, `canvasSizing.test.ts`, `provinceAnalysis.test.ts`, `decodeBmp24.test.ts`, `BmpProvinceMapSource.test.ts`, `runProvinceValidationPass.test.ts`).
