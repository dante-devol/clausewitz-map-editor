...
> **Status (2026-09-11): all of section 4 addressed except the Uint8Array/custom-protocol half of 4.3.**
...
> - 4.3 (partial): the `createImageBitmap`+canvas decode duplicated across `BmpProvinceMapSource` (used by both `useMapCanvas` and `useOverlayAssets`) and the bitmap-analysis worker is replaced by the pure `decodeBmp24` decoder (`decodeBmp24.ts`), removing the GPU/canvas round-trip and its possible color-management shift on province colors. `BmpProvinceMapSource` caches the parsed bytes by source string so loading the same image twice only parses once, while still handing each caller an independent pixel-array copy (`MapRenderer` mutates its copy in place while painting, so a shared array would leak edits between callers). `imageBitmap` is dropped from `ProvinceMapSource` entirely — it was decoded in both call sites but never actually consumed by either. **Not done:** the wire format is still base64 over IPC, not a `Uint8Array`/custom protocol — that half of the fix wasn't attempted.
